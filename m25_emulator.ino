/**
 * m25_emulator.ino - Fake M25 Wheel Simulator (RFCOMM / SPP edition)
 *
...
static void format_mac(char *out, size_t outLen, const uint8_t *mac) {
  if (!out || outLen < 18 || !mac)
    return;
  snprintf(out, outLen, "%02X:%02X:%02X:%02X:%02X:%02X", mac[0], mac[1], mac[2],
           mac[3], mac[4], mac[5]);
}
...
static bool do_process_frame(const uint8_t *raw, size_t rawLen) {
  ...
  const CmdResult result =
      command_apply(spp, sppLen, &wheel, cli_dbg(DBG_COMMANDS));

  if (result == CMD_SPEED_UPDATE)
    stats.speedUpdates++;
...
#if TRANSPORT_RFCOMM_ENABLED
  if (rfcomm_connected())
    rfcomm_send(frame, frameLen);
#endif
#if TRANSPORT_BLE_ENABLED
  if (ble_connected())
    ble_send(frame, frameLen);
#endif
}
...
static void do_send_response_for(uint8_t reqService, uint8_t reqParam) {
  ...
  const size_t frameLen =
      packet_encode_response(reqService, reqParam, &wheel, wheelCfg.key, frame);
  if (frameLen == 0) {
    Serial.println(F("[TX] ERROR: packet_encode_response failed"));
    return;
  }
  _transport_send(frame, frameLen);
}
...
static void tick_battery() {
  ...
  if (millis() - _lastBatteryMs < interval)
    return;
...
void setup() {
  ...
  // BLE transport
#if TRANSPORT_BLE_ENABLED
  ble_init(wheelCfg.deviceName, wheelCfg.bleServiceUuid, wheelCfg.bleTxUuid,
           wheelCfg.bleRxUuid);
#endif
...
void loop() {
      ...
      if (!ble_first_valid()) {
        if (ok) {
          if (cli_dbg(DBG_STALE) && ble_stale_count() > 0) {
            Serial.printf("[BLE] First valid packet after %u stale (%lums "
                          "after connect)\n",
          ...
          if (elapsed > STALE_TIMEOUT_MS) {
            ...
            Serial.printf(
                "[BLE] Timeout: %u stale packets over %lums - disconnecting\n",
                ble_stale_count(), elapsed);
            ble_disconnect();
          } else if (cli_dbg(DBG_STALE) && ble_stale_count() % 10 == 1) {
            Serial.printf(
                "[BLE] Stale: %u packets discarded, %lums after connect\n",
                ble_stale_count(), elapsed);
          }
        }
      }
  ...
  if (_ledSpeedEnabled)
    led_set_speed_indicator(wheel.speed);
